...
export class LoginComponent implements OnInit {
  ...
  check_auth(){
    ...
    this.http.get<any>('https://8000-aqua-silverfish-gv5meqbl.ws-eu16.gitpod.io/api/validate/auth', httpOptions).map(res => res).subscribe(data => {
      if (data == 1) {
        console.log('success...');
  ...
  login(){
    ...
    let postData = new FormData();
      ...
      postData.append('password',this.password);
       this.http.post<any>('https://8000-aqua-silverfish-gv5meqbl.ws-eu16.gitpod.io/api/login', postData).map(res => res).subscribe(data => {
        console.log(data);
